...
function order_goods($order_id)
{
	...
	//手机专享价格  app  jx  
	$sql = "SELECT rec_id, og.goods_id, og.product_id,og.goods_name, g.goods_thumb, og.goods_sn,og.goods_cw,og.cwnum, og.market_price,og.exclusive, og.goods_number, " .
...
function order_fee($order, $goods, $consignee)
{
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		$GG=unserialize($shipping_info['configure']);
		$haha=$GG[3]['value'];
        $hh=$total['goods_price'];
        $yunfei= $GLOBALS['_CFG']['yunfei'];
       //echo $hh.'$haha'.$yunfei;
...
/* 代码增加_end  By www.68ecshop.com */
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn,c.goods_cw,c.cwnum, c.goods_number, c.market_price, " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn,g.goods_cw,g.cwnum, g.is_on_sale, g.is_real, ".
    ...
    $parent = array(
        ...
        'goods_cw'      => addslashes($goods['goods_cw']),
        'cwnum'      => addslashes($goods['cwnum']),
...
/**
 * 取得用户信息
 * @param   int     $mobile  用户电话
 * @return  array   用户信息
 */
function user_info_mobile($mobile)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('users') .
...
/**
 * 会员转移
 * @param   string  $from_order_sn  主会员
 * @param   string  $to_order_sn    从会员
 * @return  成功返回true，失败返回错误信息
 */
function merge_users($from_order_sn, $to_order_sn)
{

    /* 订单号不能为空 */
    if (trim($from_order_sn) == '' || trim($to_order_sn) == '')
    {
        return '请填写要转移的会员的手机号！';
    }

    /* 订单号不能相同 */
    if ($from_order_sn == $to_order_sn)
    {
        return '要转移的会员的手机号不能相同！';
    }

    /* 取得从会员信息 */
    $from_order = user_info_mobile($from_order_sn);
    /* 取得主会员信息 */
    $to_order   = user_info_mobile($to_order_sn);

    /* 检查是否存在 */
    if (!$from_order)
    {
        return sprintf('会员 %s 不存在', $from_order_sn);
    }
    elseif (!$to_order)
    {
        return sprintf('会员 %s 不存在', $to_order_sn);
    }


    /* 会员转移 */
    /* 信息赋值 */
    $users_form['gf_num']=$from_order['gf_num'];
    $users_form['fh_num']=$from_order['fh_num'];
    $users_form['fh_num_status']=$from_order['fh_num_status'];
    $users_form['credit']=$from_order['credit'];
    $users_form['credit2']=$from_order['credit2'];
    $users_form['user_money']=$from_order['user_money'];
    $users_form['frozen_money']=$from_order['frozen_money'];
    $users_form['pay_points']=$from_order['pay_points'];
    $users_form['rank_points']=$from_order['rank_points'];
    $users_form['user_rank_points']=$from_order['user_rank_points'];
    $users_form['is_vip']=$from_order['is_vip'];
    $users_form['status']=$from_order['status'];
    $users_form['parent_id']=$from_order['parent_id'];
    /* 更改主会员信息 */
    update_user($to_order['user_id'],$users_form);
    /* 把会员上级由从会员改为主会员 */
    /*$down=down_list($from_order['user_id']);
    foreach($down as &$item){
        $users_to['parent_id']=$to_order['user_id'];
        update_user($item['user_id'],$users_to);
    }*/
