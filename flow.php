...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if($_SESSION['user_id'] > 0){
        /* 获取用户手机号 */
        $sql = "SELECT user_id, mobile_phone, email FROM " . $ecs->table('users') . " WHERE user_id='$_SESSION[user_id]' LIMIT 1";
        $row = $db->getRow($sql);

        /* 是否开启强制手机绑定 */
        if ($_CFG['need_mobile'] == '1' && (!$row || $row['mobile_phone'] == ''))
        {
            show_message('下订单前必须先绑定手机！', '绑定手机号', 'security.php?act=mobile_binding', 'info');
        }

        /* 是否开启强制邮箱绑定 */
        if ($_CFG['need_email'] == '1' && (!$row || $row['email'] == ''))
        {
            show_message('下订单前必须先绑定邮箱！', '绑定邮箱', 'security.php?act=email_binding', 'info');
        }
    }
    //weijie end
...
{	
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
	                "order_id, goods_id, goods_name, goods_sn,goods_cw,cwnum, product_id, goods_number, market_price, cost_price, ".
	                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id) ".
	            " SELECT '$new_order_id', goods_id, goods_name, goods_sn,goods_cw,cwnum, product_id, goods_number, market_price, cost_price, ".
	                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id ".
	            " FROM " .$ecs->table('cart') .
...
//                    }
    ...
    if($order['user_id']) pushUserMsg($order['user_id'], array('order_sn'=>$order['order_sn']));
...
else
{
   ...
   //var_dump($_COOKIE['one_step_buy']);die;
    if(empty($flow_type))
    {
    	/* 标记购物流程为普通商品 */
...
function add_gift_to_cart($act_id, $id, $price)
{
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . " (" .
                "user_id, session_id, goods_id, goods_sn,goods_cw,cwnum, goods_name, market_price, goods_price, ".
                "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) ".
            "SELECT '$_SESSION[user_id]', '" . SESS_ID . "', goods_id, goods_sn,goods_cw,cwnum, goods_name, market_price, ".
