...
$supplier_id = isset($_SESSION['supplier_id']) ? intval($_SESSION['supplier_id']) : intval($_REQUEST['supplier_id']);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id']." AND type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,supplier_id)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            ...
            '$use_enddate',
            '$_POST[send_type]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "','$_SESSION[supplier_id]')";
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        if (add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html']))
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed, supplier_id) " .
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_SUCCEED. ", '$_SESSION[supplier_id]')";
            $db->query($sql);
        }
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed, supplier_id) " .
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_FAIL. ", '$_SESSION[supplier_id]')";
            $db->query($sql);
        }
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        ...
        $db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn, supplier_id) VALUES('$bonus_typeid', '$bonus_sn', '$_SESSION[supplier_id]')");
...
function get_type_list()
{
    ...
    $sql = "SELECT bonus_type_id, COUNT(*) AS used_count".
            " FROM " .$GLOBALS['ecs']->table('user_bonus') .
            " WHERE supplier_id = ".$_SESSION['supplier_id']." AND used_time > 0".
            " GROUP BY bonus_type_id";
    $res = $GLOBALS['db']->query($sql);
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id'];
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id']." ORDER BY $filter[sort_by] $filter[sort_order]";
...
function add_to_maillist($username, $email, $subject, $content, $is_html)
{
    ...
    $sql = "INSERT INTO "  . $GLOBALS['ecs']->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send, supplier_id) VALUES ('$email', $template_id, '$content', 1, '$time', '$_SESSION[supplier_id]')";
    $GLOBALS['db']->query($sql);
    return true;
}
