...
function action_default ()
{
	...
	$smarty->assign('shu', get_user_shu($user_id)); // 获取当前用户的交易记录
	 /*@lk 2017.9.20*/                                               // print_r(get_user_reminding($user_id));
        ...
        if(!empty($user_id1)){
            ...
            if(!empty($row3)){
                ...
                if(empty($row3['is_vip'])){
					...
					$ordertt['change_desc']='一级会员进货的'.$commission1.'%';
                ...
                if(!empty($user_id1)&&empty($user_id1['is_vip'])){
                    ...
                    if(!empty($row4)){
                        ...
                        if($count>=$yjnum){
                            ...
                            $valuell=$GLOBALS['db']->getAll ("select * from " . $GLOBALS['ecs']->table('users') . " where parent_id = " . $user_id2);
                            ...
                            if($n>=$yjnum){
                                ...
                                $ordertt1['change_time']=time();
                                $ordertt1['change_desc']='二级会员进货的'.$commission2.'%';
                                $ordertt1['change_type']=3;
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $ordertt1, 'INSERT');
                            }
                        }
...
function action_account_detail ()
{
	...
	//个人总分红
    $total_fh=$db->GetOne('SELECT credit FROM ' . $ecs->table('users') . " WHERE  user_id=$user_id ");
...
function action_account_log ()
{
   ...
   /* file_put_contents('__DIR__' . 'a.php',$total_jifen);*/
	// 获取余额记录
...
function action_act_account ()
{
    ...
    if($surplus['amount'] < 50)
    {

        show_message('最低提现额度为50元');
    }
	/* 退款申请的处理 */
