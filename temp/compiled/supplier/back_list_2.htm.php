...
<div class="form-div">
  <form action="javascript:searchOrder()" name="searchForm">
    ...
    售后状态<select name="order_type" id="order_type"><option value="0">全部</option><option value="3" <?php if ($this->_var['filter']['order_type'] == '3'): ?>selected<?php endif; ?>>已完成</option><option value="2" <?php if ($this->_var['filter']['order_type'] == '2'): ?>selected<?php endif; ?>>未完成</option><option value="4" <?php if ($this->_var['filter']['order_type'] == '4'): ?>selected<?php endif; ?>>已取消</option></select>
    退单类型<select name="back_type" id="back_type"><option value="0">全部</option><option value="4" <?php if ($this->_var['filter']['back_type'] == '4'): ?>selected<?php endif; ?>>退款</option><option value="1" <?php if ($this->_var['filter']['back_type'] == '1'): ?>selected<?php endif; ?>>退货</option></select>
...
?>
  ...
  <td><input type="checkbox" name="back_id[]" value="<?php echo $this->_var['back']['back_id']; ?>" /><?php echo $this->_var['back']['back_id']; ?></td>
	...
	<td >
	<?php $_from = $this->_var['back']['goods_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'goods_info');if (count($_from)):
    foreach ($_from AS $this->_var['goods_info']):
?>
	[ ID：<?php echo $this->_var['goods_info']['goods_id']; ?> ] &nbsp; &nbsp; 
	<a href="../goods.php?id=<?php echo $this->_var['goods_info']['goods_id']; ?>" target="_blank"><?php echo $this->_var['goods_info']['goods_name']; ?></a><br />
