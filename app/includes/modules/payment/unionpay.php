	...
	*/
	function get_code($order, $payment)
	{
		...
		$params = array(
			...
			'txnSubType' => '01',				//交易子类
			'bizType' => '000201',				//业务类型
			'frontUrl' =>  return_url_app(basename(__FILE__, '.php')),  		//前台通知地址
			'backUrl' => notify_url_app(basename(__FILE__, '.php')),		//后台通知地址	
			'signMethod' => '01',		//签名方法
	...
	*/
	function respond()
	{
		...
		$params = array(
			...
			'frontUrl' =>  return_url_app(basename(__FILE__, '.php')),  		//前台通知地址
			'backUrl' => return_url_app(basename(__FILE__, '.php')),		//后台通知地址	
...
function getCertId($cert_path,$pwd) {
	$pkcs12certdata = file_get_contents ( ROOT_PATH.$cert_path );
...
function sign_union(&$params,$cert_path,$pwd) {
	...
	// 签名证书路径
	$pkcs12 = file_get_contents ( ROOT_PATH.$cert_path );
...
eot;
	...
	$html .= <<<eot
    <input type="submit" type="hidden" value="立即支付">
    </form>
eot;
	return $html;
}
