...
$ui_arr = array(
    ...
    'get_password','reset_password','booking_list','add_booking','account_raply','account_raply1','account_deposit','account_deposit1','account_deposit2','account_log',
    'account_detail','account_detail1','account_detail2','account_detail3','act_account','pay','default','bonus','group_buy','group_buy_detail','affiliate','comment_list','account_log1',
    ...
    'supplier_reg','comment_order','address','set_address','follow_shop','account_manage','account_manage1','my_comment','code','shaidan_send'
);
...
/* 我的股份 */
function action_account_manage1(){
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    $smarty = $GLOBALS['smarty'];
    ...
    $ecs = $GLOBALS['ecs'];
    $user_id = $GLOBALS['user_id'];
    $ex_where1 = " and user_id=$user_id and gufennum >=1 and is_paid=1 and status=1" ;
    $total_gufen=$db->GetOne('SELECT SUM(gufennum) FROM ' . $ecs->table('user_account') . " WHERE 1 $ex_where1 ");
    $sql = "SELECT value FROM ".$ecs->table('shop_config')." WHERE code='affiliate'";
    $affiliate = $db->getOne($sql);
    $affiliate = unserialize($affiliate);
    if(empty($total_gufen))
    {
        $total_gufen = 0;
    }
    $affiliate_intro = nl2br(sprintf($_LANG['affiliate_intro'][3], $affiliate['config']['expire'], $_LANG['expire_unit'][$affiliate['config']['expire_unit']], $affiliate['config']['level_register_all'], $affiliate['config']['level_register_up'], $affiliate['config']['level_money_all'], $affiliate['config']['level_point_all']));
...
function action_default()
{
    ...
    $db = $GLOBALS['db'];
    $ecs = $GLOBALS['ecs'];
    $data_start=strtotime(date('Y-m-d 00:00:00',time()))-28800;
    $data_end=time();
    //file_put_contents(__DIR__.'/aah.php',$data_end.'a'.$data_start);
    //$ex_where1 = " and shipping_time_end>=$data_start and shipping_time_end <=$data_end " ;
    ...
    $ex_where2 = " and user_id=$user_id and change_type=3 " ;
    ...
    //进货累计2万积分过的一个分红权
    $jhleiji=$_CFG['jhleiji'];
    $order_count['finished1'] = $db->GetOne('SELECT SUM(goods_amount)  FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('finished'));
    ...
    if($row1['fh_num']>=100){
        $order1['fh_num']=100;
    }elseif ($row1['fh_num']<=0){
        $order1['fh_num']=0;
    }
    {
        $order1['fh_num']=$row1['fh_num']+1;
    }
    //ile_put_contents(__DIR__.'/a.php',$data_start);
    if(empty($row1['fh_num'])&&$row1['pay_points']>$jhleiji){

        $order1['fh_num']=$row1['fh_num']+1;
        $order1['pay_points']=$row1['pay_points']-$jhleiji;
        $ordertt1['user_id']=$user_id;
        $ordertt1['fh_num']=1;
        $ordertt1['pay_points']=-$jhleiji;
        $ordertt1['user_money']=$ordertt1['rank_points']=$ordertt['frozen_money']=$ordertt['credit']=0;
        $ordertt1['change_time']=time();
        $ordertt1['change_desc']='减少积分增加分红权';
    ...
    if(!empty($row1['fh_num'])&&$row1['pay_points']>5000){
        $up_fh=floor($row1['pay_points']/5000);
        $order1['fh_num']=$row1['fh_num']+$up_fh;
        $order1['pay_points']=$row1['pay_points']-(5000*$up_fh);
        $ordertt1['user_id']=$user_id;
        $ordertt1['fh_num']=$up_fh;
        $ordertt1['pay_points']=-(5000*$up_fh);
        $ordertt1['user_money']=$ordertt1['rank_points']=$ordertt['frozen_money']=$ordertt['credit']=0;
        $ordertt1['change_time']=time();
        $ordertt1['change_desc']='减少积分增加分红权';
        $ordertt1['change_type']=3;
    ...
    //平台总积分
    //$total_jifen=$db->GetOne('SELECT SUM(goods_amount) FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where1 ");
    $total_jifen=$db->GetOne('SELECT SUM(pay_points) FROM ' . $ecs->table('account_log') . " WHERE 1 $ex_where1 ");
    $smarty->assign('total_jifen', $total_jifen);
    $total_fh=$db->GetOne('SELECT SUM(user_money) FROM ' . $ecs->table('account_log') . " WHERE 1 $ex_where2 ");
    $smarty->assign('total_fh', $total_fh);
...
function action_code()
{
    ...
    $http='www.158abc.com';//获取当前域名
    $url ='http://'.$http. '/mobile/register.php?id='.$user_id;
    $qrcode_path = ROOT_PATH . 'themesmobile/68ecshopcom_mobile/images/qr/' . $user_id . '.png';
    $qrbg_path = ROOT_PATH . 'themesmobile/68ecshopcom_mobile/images/qrbg.png';
    if (!is_file($qrcode_path)){
        ...
        QRcode::png($url,$qrcode_path,QR_ECLEVEL_L,$matrixPointSize,2,false);
        //file_put_contents('__DIR__'.'a.php',ROOT_PATH);
        //合并图片
        $QR = imagecreatefromstring(file_get_contents($qrcode_path));
        $qrbg_path = imagecreatefromstring(file_get_contents($qrbg_path));
        $QR_width = imagesx($QR);//二维码图片宽度
        $QR_height = imagesy($QR);//二维码图片高度
        $logo_width = imagesx($qrbg_path);//背景图片宽度
        $logo_height = imagesy($qrbg_path);//背景图片高度
        //重新组合图片并调整大小
        imagecopyresampled($qrbg_path, $QR, 240, 536, 0, 0, 255, 255, $QR_width, $QR_height);

        $font = ROOT_PATH . 'themesmobile/68ecshopcom_mobile/images/msyhbd.ttf';
//        echo $font;
        $black = imagecolorallocate($qrbg_path, 0, 0, 0);
        imagefttext($qrbg_path, 20, 0, 270, 960, $black, $font, $user_id);
        //输出图片
        imagepng($qrbg_path, $qrcode_path);
        imagedestroy($qrbg_path);
    }
    //$qrcode_img = Request::instance()->domain() . '/mobile/themesmobile/68ecshopcom_mobile/qr/' . $user_id . '.png';
    echo '<img src="http://'.$http.'/mobile/themesmobile/68ecshopcom_mobile/images/qr/' . $user_id . '.png" style="width:100%;">';
//  	$info['qr'] = $qrcode_img;
//    $info['url'] = $url;
...
function action_affirm_received()
{
    ...
    if(affirm_received($order_id, $user_id))
    {
        ...
        //获取订单信息和个人信息@xqb 2017.7.26
        $commission1=$_CFG['commission1']/100;
        $commission2=$_CFG['commission2']/100;
        $yjnum=$_CFG['yjnum'];
        $yjlj=$_CFG['yjlj'];
        $sql1 = "SELECT * FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
        $row1 = $GLOBALS['db']->getRow($sql1);
        //$sql2 = "SELECT * FROM " .$GLOBALS['ecs']->table('order_info'). " WHERE $order_id = '$order_id'";
        $row2 = $row = $GLOBALS['db']->getRow("SELECT o.order_sn,u.parent_id, o.is_separate,(o.goods_amount - o.discount) AS goods_amount, o.user_id,o.supplier_id  FROM " . $GLOBALS['ecs']->table('order_info') . " o"." LEFT JOIN " . $GLOBALS['ecs']->table('users') . " u ON o.user_id = u.user_id"." WHERE order_id = '$order_id'");
        $user_id1=$row1['parent_id'];
        //上级分销商
        if(!empty($user_id1)){
            $sql3 = "SELECT * FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id1'";
            $row3 = $GLOBALS['db']->getRow($sql3);
            if(!empty($row3)){
                //获取一级会员订单的$commission1的奖励
                if(empty($row3['is_vip'])){
                    $order1['pay_points']=($row2['goods_amount']*$commission1)+$row3['pay_points'];
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $order1, "UPDATE", "user_id = $user_id1");
                    $ordertt['user_id']=$user_id1;
                    $ordertt['pay_points']=$row2['goods_amount']*$commission1;
                    $ordertt['rank_points']=$ordertt['frozen_money']=$ordertt['user_money']=0;
                    $ordertt['change_time']=time();
                    $ordertt['change_desc']='一级会员进货的'.$_CFG['commission1'].'%';
                    $ordertt['change_type']=3;
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $ordertt, 'INSERT');
                }
                $user_id2=$row3['parent_id'];
                //获取二级会员订单的$commission2的奖励
                if(!empty($user_id1)&&empty($user_id1['is_vip'])){
                    $sql4 = "SELECT * FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id2'";
                    $row4 = $GLOBALS['db']->getRow($sql4);
                    if(!empty($row4)){
                        $count=$GLOBALS['db']->getOne("select count(*) from " . $GLOBALS['ecs']->table('users') . " where parent_id = " . $user_id2);
                        /* file_put_contents('__DIR__'.'a.php',$count);*/
                        if($count>=$yjnum){

                            $valuell=$GLOBALS['db']->getAll ("select count(*) from " . $GLOBALS['ecs']->table('users') . " where parent_id = " . $user_id2);
                            foreach ($valuell as &$val){
                                $user_idval=$val['user_id'];
                                $ex_where = " and user_id=$user_idval and extension_code <> 'virtual_good'" ;
                                $order_count['finished1'] = $db->GetOne('SELECT SUM(goods_amount)  FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('finished'));
                                if($order_count['finished1']>=$yjlj){
                                    $n=$n+1;
                                }
                            }
                            if($n>=$yjnum){
                                $order2['pay_points']=($row2['goods_amount']*$commission2)+$row4['pay_points'];
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $order2, "UPDATE", "user_id = $user_id2");
                                $ordertt1['user_id']=$user_id2;
                                $ordertt1['pay_points']=$row2['goods_amount']*$commission2;
                                $ordertt1['rank_points']=$ordertt['frozen_money']=$ordertt['user_money']=0;
                                $ordertt1['change_time']=time();
                                $ordertt1['change_desc']='二级会员进货的'.$_CFG['commission2'].'%';
                                $ordertt1['change_type']=3;
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $ordertt1, 'INSERT');
                            }
                        }
...
function action_account_raply()
{
    ...
    $ecs = $GLOBALS['ecs'];
    $user_id = $GLOBALS['user_id'];
    $sql = "SELECT credit FROM " .$GLOBALS['ecs']->table('users').
        " WHERE user_id = '$user_id'";
    $credit=$GLOBALS['db']->getOne($sql);

    $smarty->assign('txshouxu', $_CFG['txshouxu']);
    $smarty->assign('credit', $credit);
    $smarty->assign('user_info', get_user_info());
    $smarty->display('user_transaction.dwt');
}
/* 股份分红提现界面 */
function action_account_raply1()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    $ecs = $GLOBALS['ecs'];
    $user_id = $GLOBALS['user_id'];
    $sql = "SELECT credit2 FROM " .$GLOBALS['ecs']->table('users').
        " WHERE user_id = '$user_id'";
    $credit=$GLOBALS['db']->getOne($sql);

    $smarty->assign('txshouxu', $_CFG['gftxsx']);
    $smarty->assign('credit', $credit);
    $smarty->assign('user_info', get_user_info());
    $smarty->display('user_transaction.dwt');
}
...
/* 股份充值界面 */
function action_account_deposit1()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
...
/* VIP会员充值界面 */
function action_account_deposit2()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    $surplus_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    $account = get_surplus_info($surplus_id);
    $num['daynumber1']=$_CFG['money1'];
    $num['daynumber2']=$_CFG['moregu'];
    $num['daynumber3']=$_CFG['moshi'];
    $smarty->assign('num', $num);
    // 获取剩余余额
    $sql = "SELECT user_money FROM " .$GLOBALS['ecs']->table('users').
        " WHERE user_id = '$user_id'";
    $sqll = "SELECT value FROM ".$ecs->table('shop_config')." WHERE code='affiliate'";
    $affiliate = $db->getOne($sqll);
    $affiliate = unserialize($affiliate);
    $affiliate_intro = nl2br(sprintf($_LANG['affiliate_intro'][4], $affiliate['config']['expire'], $_LANG['expire_unit'][$affiliate['config']['expire_unit']], $affiliate['config']['level_register_all'], $affiliate['config']['level_register_up'], $affiliate['config']['level_money_all'], $affiliate['config']['level_point_all']));
    $smarty->assign('affiliate_intro', $affiliate_intro);
    $surplus_amount = $GLOBALS['db']->getOne($sql);;
    $sqlll = "SELECT COUNT(*) FROM " . $ecs->table('user_account') . " WHERE user_id = '$user_id'" . " AND vip=1 and qixian=1";
    $counthh = $db->getOne($sqlll);
    $smarty->assign('counthh', $counthh);
...
/* 股份明细界面 */
function action_account_detail1()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    /* 获取记录条数 */
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('user_account') . " WHERE user_id = '$user_id'" . " AND is_paid=1 and gufennum>=1";
    $record_count = $db->getOne($sql);
    ...
    // 获取剩余余额
    $ex_where1 = " and user_id=$user_id and gufennum >=1 and is_paid=1 and status=1" ;
    $surplus_amount=$db->GetOne('SELECT SUM(gufennum) FROM ' . $ecs->table('user_account') . " WHERE 1 $ex_where1 ");
    //$surplus_amount = get_user_surplus($user_id);
    if(empty($surplus_amount))
    {
        $surplus_amount = 0;
    ...
    // 获取余额记录
    $account_log = array();
    $sql = "SELECT * FROM " . $ecs->table('user_account') . " WHERE user_id = '$user_id'" . " AND is_paid=1 and gufennum>=1 and status=1 " . " ORDER BY id DESC";
    $res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
    while($row = $db->fetchRow($res))
    {

        $row['change_time'] = local_date($_CFG['date_format'], $row['paid_time']);
        $row['short_change_desc'] = sub_str($row['user_note'], 60);
        if($row['qixian']==6){
            $row['shijian']='半年';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+6 month",$row['paid_time']));
        }else if($row['qixian']==12){
            $row['shijian']='一年';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+12 month",$row['paid_time']));
        }else if($row['qixian']==60){
            $row['shijian']='五年';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+60 month",$row['paid_time']));
        }
        $account_log[] = $row;
    }
/*file_put_contents(__DIR__.'/a.php',json_encode($account_log));*/
    // 模板赋值
...
/* VIP购买明细界面 */
function action_account_detail3()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    /* 获取记录条数 */
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('user_account') . " WHERE user_id = '$user_id'" . " AND is_paid=1 and vip=1 and status=1  ";
    $record_count = $db->getOne($sql);
    ...
    // 获取余额记录
    $account_log = array();
    $sql = "SELECT * FROM " . $ecs->table('user_account') . " WHERE user_id = '$user_id'" . " AND is_paid=1 and vip=1 and status=1 " . " ORDER BY id DESC";
    $res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
    while($row = $db->fetchRow($res))
    {

        $row['change_time'] = date("Y-m-d",$row['add_time']);
        $row['short_change_desc'] = sub_str($row['user_note'], 60);
        if($row['qixian']==6){
            $row['shijian']='半年';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+6 month",$row['add_time']));
        }else if($row['qixian']==12){
            $row['shijian']='一年';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+12 month",$row['add_time']));
        }else if($row['qixian']==1){
            $row['shijian']='1个月';
            $row['end_time'] = $end=date("Y-m-d",strtotime("+1 month",$row['add_time']));
        }
        $account_log[] = $row;
    }
...
/* 股份分红明细界面 */
function action_account_detail2()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    // 获取余额记录
    $ex_where1 = " and user_id=$user_id and change_type =4 " ;
    $account_log=$db->GetAll('SELECT * FROM ' . $ecs->table('account_log') . " WHERE 1 $ex_where1 "."order by change_time desc");
    foreach($account_log as &$value){
        $value['shijian']=local_date($_CFG['date_format'], $value['change_time']);
    }
...
function action_act_account()
{
    ...
    $mobile = $_POST['mobile'];
    $idcard = $_POST['idcard'];
    $daynumber = $_POST['daynumber'];//众筹月数
    $cz_type = $_POST['cz_type'];//充值方式为股份充值
    if($amount <= 0)
    {
    ...
    {
        if($surplus['amount'] < $_CFG['txbz'])
        {
            show_message('最低提现额度为'.$_CFG['txbz'].'元');
        }
        if(empty($realname))
        {
        ...
        /* 判断是否有足够的余额的进行退款的操作 */
        if($cz_type == 1){
            $sur_amount = $GLOBALS['db']->getOne("SELECT credit2 FROM " .$GLOBALS['ecs']->table('users').
                " WHERE user_id = '$user_id'");
        }else{
            $sur_amount = $GLOBALS['db']->getOne("SELECT credit FROM " .$GLOBALS['ecs']->table('users').
                " WHERE user_id = '$user_id'");
        }
        ...
        if($cz_type == 1){
            $money=round($surplus['amount']*(100-$_CFG['gftxsx']));
        }else{
            $money=round($surplus['amount']*(100-$_CFG['txshouxu']));
        }
        ...
        $mch_appid='wx2a43b0a4c385df3d';//公众账号appid

        $mchid='1247934101';//商户号

        $nonce_str='qyzf'.rand(100000, 999999);//随机数

        $partner_trade_no=$surplus['rec_id'];//商户订单号

        $openid=$_SESSION['wxid'];//用户唯一标识,上一步授权中获取
        ...
        $spbill_create_ip='114.55.105.33';//请求ip
        //企业打款到个人
        $dataArr=array();
        ...
        $dataArr['mch_appid']=$mch_appid;//商户的应用appid

        $dataArr['mchid']=$mchid;//商户ID
...
//拼写正确的xml参数
        ...
        $zs1="./cert/apiclient_cert.pem";

        $zs2="./cert/apiclient_key.pem";
        ...
        curl_close ( $ch );
        $array_data = json_decode(json_encode(simplexml_load_string($info, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
        /*echo "-----<br/>请求返回值：";
        file_put_contents(__DIR__.'/a1.php',$info);
        print_r($array_data);
		echo "<br/>*****";die;*/

        if($array_data['return_code']=="SUCCESS"&&$array_data['result_code']=="SUCCESS"){
            $txorderid=$array_data['partner_trade_no'];
            $order1['admin_user']='admin';
            $order1['paid_time']=time();
            $order1['admin_note']='微信订单号：'.$array_data['payment_no'].'时间：'.$array_data['payment_time'];
            $order1['payment']='微信支付';
            $order1['is_paid']=1;
            if($cz_type == 1){
                $order2['credit2']=$sur_amount-$surplus['amount'];
                $order1['status']=1;
            }else{
                $order2['credit']=$sur_amount-$surplus['amount'];
            }
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_account'), $order1, "UPDATE", "id = $txorderid");


            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $order2, "UPDATE", "user_id = $user_id");
            show_message('提现成功');
        }
    ...
    {
        if($surplus['payment_id'] <= 0 && $cz_type!=2)
        {
        ...
        $sur_amount2 = $GLOBALS['db']->getOne("SELECT user_money FROM " .$GLOBALS['ecs']->table('users').
            " WHERE user_id = '$user_id'");
        if($cz_type == 1){

            if(floor($gf_num)==$gf_num){
                //判断股份数量是否为整数
            }else{
                show_message('股份数量输入大于“0”的整数');
            }
            ...
            {
                $surplus['gufennum']=$gf_num;
                $surplus['qixian']=$daynumber;
                $surplus['name']=$name;
                $surplus['mobile']=$mobile;
                $surplus['idcard']=$idcard;
                // 插入会员账目明细
                $surplus['rec_id'] = insert_user_account1($surplus, $amount);
            }
        }else if($cz_type == 2){

            if(empty($daynumber))
            {
                show_message('请选取会员时间');
            }
            if($sur_amount2<$surplus['amount'])
            {
                show_message('您的可用余额不足请充值后，再来！','','user.php?act=account_deposit&type=1');

            }

            $order2['is_vip']=1;
            $order2['user_rank_points']=5000;
            if($daynumber==1){
                $order2['pass_time']=strtotime("+1 month",gmtime());
            }else if($daynumber==6){
                $order2['pass_time']=strtotime("+6 month",gmtime());
            }else{
                $order2['pass_time']=strtotime("+12 month",gmtime());
            }

            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $order2, "UPDATE", "user_id = $user_id");
            log_account_change($user_id, -$amount, 0, 0, 0, '购买会员扣除', 99);

            $sql = "insert into ".$GLOBALS['ecs']->table('user_account')." (`user_id`,`amount`,`add_time`,`qixian`,`process_type`,`payment`,`is_paid`,`status`,`tuigutime`,`vip`)
			value ({$surplus['user_id']},'{$amount}','".gmtime()."','{$daynumber}','1','余额支付','1','1','{$order2['pass_time']}','1')";
            $GLOBALS['db']->query($sql);

                $surplus['rec_id'] = $GLOBALS['db']->insert_id();
            if($surplus['rec_id']){
                show_message('购买成功！','','user.php');
            }
            ...
            //file_put_contents(__DIR__.'/aa1.php',json_encode($surplus));
        }
        ...
        $smarty->assign('gufennum', $gf_num);
        $smarty->assign('realname', $name);
        $smarty->assign('mobile', $mobile);
        $smarty->assign('idcard', $idcard);
        $smarty->assign('cz_type', $cz_type);
        $smarty->assign('order', $order);
        $smarty->display('user_transaction.dwt');
    }
}
...
//签名步骤二：在string后加入KEY
    $String = $String."&key=20152016zxcvbnm20152016zxcvbnm20";
    ...
    if(empty($goodsid))
    {
        ...
        if(empty($affiliate['config']['separate_by']))
        {
            ...
            for($i = 1; $i <= $num; $i ++)
            {
                ...
                $affdb[1]['money'] =$_CFG['commission1'] ;
                $affdb[2]['money'] =$_CFG['commission2'] ;
            }
