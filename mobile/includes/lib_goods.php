...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['final_price'] = price_format($row['org_price']);
...
function cut_log($act_id)
{
    $log = array();
    $sql = "SELECT a.* " .
            "FROM " . $GLOBALS['ecs']->table('users_activity') . " AS a " .
            "WHERE act_id = '$act_id' " .
...
function user_cut_log($user_id,$act_id, $page = 1)
{
    ...
    /* 取得砍价记录列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('cut_log').
    ...
    $log = array();
    $sql = "SELECT c.* " .
            "FROM " . $GLOBALS['ecs']->table('cut_log') . " AS c  " .
            "LEFT JOIN " .  $GLOBALS['ecs']->table('users_activity') . " AS u ON (u.user_id = c.act_user and u.act_id=c.act_id) " .
