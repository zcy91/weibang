 ...
 * @return  int
 */
function insert_user_account1($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
        ' (user_id,gufennum, admin_user, amount, add_time,qixian, paid_time, admin_note, user_note, process_type, payment, is_paid,status,realname,mobile,idcard)'.
        " VALUES ('$surplus[user_id]',$surplus[gufennum], '', '$amount', '".gmtime()."',$surplus[qixian], 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,1,'$surplus[name]','$surplus[mobile]','$surplus[idcard]')";
    $GLOBALS['db']->query($sql);

    return $GLOBALS['db']->insert_id();
}
function insert_user_account2($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
        ' (user_id,gufennum, admin_user, amount, add_time,qixian, paid_time, admin_note, user_note, process_type, payment, is_paid,status,vip)'.
        " VALUES ('$surplus[user_id]',$surplus[gufennum], '', '$amount', '".gmtime()."',$surplus[qixian], 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,1,'$surplus[vip]')";
    $GLOBALS['db']->query($sql);
 ...
 * @return  int
 */
function update_user_account1($surplus)
{
    $sql = 'UPDATE ' .$GLOBALS['ecs']->table('user_account'). ' SET '.
        "amount     = '$surplus[amount]', ".
        "user_note  = '$surplus[user_note]', ".
...
{   //@xqb 2017.7.25
    ...
    $info['nickname']  = $GLOBALS['db']->getOne("SELECT nickname FROM ".$GLOBALS['ecs']->table('weixin_user')." WHERE ecuid='$user_id'");
    ...
    $info['fh_num'] = $row['fh_num'];
    $info['credit'] = $row['credit'];
    $info['fh_num_status'] = $row['fh_num_status'];
    $info['totalfh_num'] =  $GLOBALS['db']->getOne("SELECT sum(fh_num) FROM ".$GLOBALS['ecs']->table('users')." WHERE 1");
...
function get_rank_info()
{
    ...
    if (!empty($_SESSION['user_rank']))
    {
        ...
        {
            $user_rank = $db->getOne("SELECT user_rank_points FROM " . $ecs->table('users') . " WHERE user_id = '$_SESSION[user_id]'");
