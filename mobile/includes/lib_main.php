...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.user_rank_points, u.rank_points, '.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['user_rank_points']) . ' AND max_points > ' . intval($row['user_rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.credit2, u.gf_num, u.user_money, u.pay_points, u.reg_time'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
