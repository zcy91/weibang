 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: cut.php 17217 2016-01-10 06:29:08Z PRINCE QQ 113549093 $
 */
...
/*------------------------------------------------------ */
//-- act 操作项的初始化  PRINCE QQ 113549093
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('cut.dwt', $cache_id))
    {
        ...
        $cut['share_url'] ='http://'.$_SERVER['HTTP_HOST']."/mobile/cut.php?act=view&id=".$id."&actuid=".$actuid."&u=".$_SESSION['user_id']; //20160119 added by PRINCE 113549093
		...
		if(strpos($goods['goods_thumb'],'ttp')>0){
		...
		$weixin_config = $db->getRow("SELECT  *  FROM ".$GLOBALS['ecs']->table('weixin_config') );
        $smarty->assign('fenxiao_url',    $weixin_config['tianxin_url']);  // 引导关注
		...
		$jssdk = new JSSDK($appid=$weixin_config['appid'], $weixin_config['appsecret']);
...
//生成随机数 P R I N C E Q Q 113549093
