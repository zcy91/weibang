...
elseif ($_REQUEST['act'] == 'insert_update') {
	...
	if (isset($_POST['finish'])) {
		...
		$supplier_id = $db->getOne("SELECT supplier_id FROM ".$ecs->table('goods')." where goods_id='$goods_id'");
		$pintuan = array(
			'act_name' => $act_name,
			'act_desc' => $_POST['act_desc'],
...
function pintuan_list_adm()
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr['supplier_name'] = $GLOBALS['db']->getOne("SELECT supplier_name FROM ".$GLOBALS['ecs']->table('supplier')." where supplier_id='$row[supplier_id]'");
