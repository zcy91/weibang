...
function action_default ()
{
    ...
    $smarty->assign('parent_ids', $parent_ids);
...
function action_register ()
{
	...
	$ecs = $GLOBALS['ecs'];
   // $smarty->assign('parent_ids',$parent_ids);
    //file_put_contents('__DIR__'.'a.php',$parent_ids);
	/* 增加是否关闭注册 */
	...
	else
	{
		...
		if($result)
		{
			...
			//修改新注册的用户成为普通分销商
            $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET parent_id = $rec_user WHERE user_id = '" . $_SESSION['user_id'] . "'");
