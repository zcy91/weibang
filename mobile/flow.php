...
{	
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
	                "order_id, goods_id, goods_name, goods_cw,cwnum, goods_sn, product_id, goods_number, market_price, cost_price, ".
	                "goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id) ".
	            " SELECT '$new_order_id', goods_id, goods_name, goods_cw,cwnum, goods_sn, product_id, goods_number, market_price, cost_price, ".
	                "goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id ".
	            " FROM " .$ecs->table('cart') .
		...
		{
			$sql = "UPDATE ". $ecs->table('users_activity') ." SET `order_times`=`order_times`+1 WHERE act_id=".$order['extension_id']." and user_id =".$order['user_id'];
			$db->query($sql);
		}
