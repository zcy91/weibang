...
$rs = $db->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_corn') . " WHERE `issend` = 0 and `sendtype`={$type} order by id desc" );
...
if(count($rs) > 0){
	...
	for($i = 0; $i < count($list); $i++)
	{
		...
		if($type == 1){
			if(isset($_GET['is_affiliate']) && $_GET['is_affiliate'] == 1)
			{
				 ...
				 foreach($list as $val)
				 {
					  ...
					  $db->query("UPDATE " . $GLOBALS['ecs']->table('weixin_corn') . " SET issend=2,sendtime=".mktime()." where id ={$val['id']}");
				 }
			}
			else
			{
		...
		if($user){
			...
			$db->query("UPDATE " . $GLOBALS['ecs']->table('weixin_corn') . " SET issend=2,sendtime=".mktime()." where id ={$rs['id']}");
		}
		if($_GET['ajax'] == 1){
