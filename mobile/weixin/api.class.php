...
class weixinapi{
	...
	function bindUser($wxid,$email,$pwd,$username=''){
		...
		//自动注册成功之后，把邮箱取消掉，方便用户重新绑定邮箱及账号
		$aite_id = 'weixin_'.$wxid;
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET is_fenxiao=2,aite_id='$aite_id',email='',is_validated='0' WHERE user_id = '$user_id'");
