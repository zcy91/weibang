...
/**
 * 拼团商品前台文件
 * $Author: weijie.online  $
 * $Id: pintuan.php 17217 2016-01-11 06:29:08Z weijie.online  $
 */
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if($is_weixin)
    {
        ...
        $new_pintuan = get_new_pintuan($pintuan_id);
        foreach ($new_pintuan as $key => $value) {
            // 获取距离
            if($value['latitude'] && $value['longitude'])
            {
                $distance = $geohash->getDistance($latitude, $longitude, $value['latitude'], $value['longitude']);
                $new_pintuan[$key]['distance'] = $distance;
                ...
                $new_pintuan[$key]['address'] = get_puser_address($value['latitude'], $value['longitude']);
            }
            else
            {
                $distance = "未知区域";
                $new_pintuan[$key]['distance'] = '99999999';
                $unit = '';
            }
            $new_pintuan[$key]['distance_'] = $distance.$unit;
        }

        // 按距离排序
        usort($new_pintuan, function($a, $b){
            return $a['distance'] > $b['distance'] ? 1 : -1;
        });

        $smarty->assign('is_weixin', '1');
        $smarty->assign('new_pintuan', $new_pintuan);
    }
    else
    {
        $new_pintuan = get_new_pintuan($pintuan_id);
...
function get_puser_address($lat, $lng){
    // 逆地址解析
