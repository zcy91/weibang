...
if ($_REQUEST['act'] == 'lists') {
    ...
    $nowtime = mktime();
    $supplier = $db->getAll("SELECT supplier_id FROM ".$ecs->table('goods_activity')." WHERE act_type='9' AND start_time < $nowtime AND end_time > $nowtime");
    if(count($supplier))
    {
        ...
        $suppliers_sql = implode($suppliers, ',');
    }
    $suppliers_sql = $suppliers_sql ? $suppliers_sql:'0';

    // 获取所有店铺
    $sql="SELECT supplier_id,user_id,supplier_name,latitude,longitude FROM ". $ecs->table("supplier") ." WHERE status=1 AND latitude<>'' AND longitude<>'' AND supplier_id in ($suppliers_sql)";
    ...
    foreach ($supplier_list as $key => $supplier) {
        ...
        $goods_activity = $db->getAll("SELECT goods_id FROM ".$ecs->table('goods_activity')." WHERE supplier_id=".$supplier['supplier_id']." AND act_type='9' AND start_time < $nowtime AND end_time > $nowtime");
        $goods_activitys = array();
        foreach ($goods_activity as $keya => $valuea) {
            # code...
            $goods_activitys[] = $valuea['goods_id'];
        }
        $goods_activitys_sql = implode($goods_activitys, ',');

        // 查询店铺下的商品
        $sql2 = "SELECT goods_id,goods_name,shop_price,market_price,goods_img,goods_thumb FROM ". $ecs->table("goods") ." WHERE goods_id in ($goods_activitys_sql) AND supplier_id=". $supplier['supplier_id'] ." LIMIT 3";
        $goods_list = $db->GetAll($sql2);
        foreach ($goods_list as $keys => $values)
        {
            $goods_list[$keys]['pintuan'] = $db->getRow("SELECT act_id,act_name FROM ".$ecs->table('goods_activity')." WHERE goods_id='$values[goods_id]' AND act_type = 9");
        }
    ...
    $smarty->display("pintuan_city.dwt");
}
    ...
    if ($supplier['latitude'] && $supplier['longitude']){
        ...
        $smarty->display("pintuan_map.dwt");
    }
}
