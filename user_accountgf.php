...
//获取每充值“$chongzhi”金额赠送“$chongzhifh”分红权
    $chongzhi=$GLOBALS['_CFG']['chongzhi'];
    $chongzhifh=$GLOBALS['_CFG']['chongzhifh'];
    $fhsx=$GLOBALS['_CFG']['fhsx'];
    if($row1['fh_num']>=$fhsx){
        $order1['fh_num']=$fhsx;
    }else{
        $order1['fh_num']=$row1['fh_num']+$chongzhifh;
    }
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        {
            $user_account = get_user_surplusc($account['user_id']);
            ...
            //更新会员余额数量
            log_account_change($account['user_id'],  0, 0, 0, 0, 0, $amount, $_LANG['surplus_type_1'], ACT_DRAWING);
			//是否开启余额变动给客户发短信 -提现
...
//                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
    ...
    file_put_contents(__DIR__.'/aa.php',json_encode($filter));
