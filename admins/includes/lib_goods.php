...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id, goods_name, goods_type, goods_sn,goods_cw, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
    ...
    foreach($row as $k=>$v)
    {
        ...
        $row[$k]['goods_cw'] = $v['goods_cw'] ? $v['goods_cw'] : 1;
        $res = $GLOBALS['db']->getRow('SELECT user_price FROM ' . $GLOBALS['ecs']->table('member_price') . ' WHERE goods_id = ' . $v['goods_id']);

        $row[$k]['member_price_list']= $res? $res['user_price'] : $v['shop_price'] ;
    }
