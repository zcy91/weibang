...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
            ...
            WHERE o.order_id = '$order[order_id]'order by cwnum asc";
    $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        @file_get_contents($autoUrl.'mobile/weixin/auto_do.php?type=2');
        ...
        if ($order['user_id'] > 0)
        {
            ...
            if(empty($user['is_vip'])){
                ...
                log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), 0, sprintf($_LANG['order_gift_integral'], $order['order_sn']));
            }
    ...
    if ($order['order_status'] == SS_SHIPPED_ING)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            if(empty($user['is_vip'])){
                ...
                log_account_change($order['user_id'], 0, 0, (-1) * intval($integral['rank_points']), 0, sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
            }
...
elseif ($_REQUEST['act'] == 'merge_users')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here', '会员转移');
    $smarty->assign('action_link', array('href' => 'users.php?act=list', 'text' => '会员转移'));

    /* 显示模板 */
    assign_query_info();
    $smarty->display('merge_users.htm');
}
/*------------------------------------------------------ */
//-- 合并订单
/*------------------------------------------------------ */
...
//获取订单信息和个人信息@xqb 2017.7.26
        ...
        $row2 =$GLOBALS['db']->getRow($sql2); //$row = $GLOBALS['db']->getRow("SELECT o.order_sn,u.parent_id, o.is_separate,(o.goods_amount - o.discount) AS goods_amount, o.user_id,o.supplier_id  FROM " . $GLOBALS['ecs']->table('order_info') . " o"." LEFT JOIN " . $GLOBALS['ecs']->table('users') . " u ON o.user_id = u.user_id"." WHERE order_id = '$order_id'");
        //file_put_contents(__DIR__.'/a.php',json_encode($row2));
        $user_idd=$row2['user_id'];
        $sql1 = "SELECT * FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_idd'";
...
//-- 会员转移
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'ajax_merge_users')
{
    /* 检查权限 */
    admin_priv('order_os_edit');
    include_once(ROOT_PATH . 'includes/cls_json.php');
    ...
    $m_result = merge_users($from_order_sn, $to_order_sn);
    $result = array('error'=>0,  'content'=>'');
    if ($m_result === true)
    {
        $result['message'] = $GLOBALS['_LANG']['act_ok'];
