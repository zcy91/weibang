...
if ($_REQUEST['act'] == 'list')
{
    ...
    $user['gufen_num']=$total_gufen;
    //file_put_contents(__DIR__.'/aa.php',json_encode($user));
    $smarty->assign('user', $user);

    if (empty($_REQUEST['account_type']) || !in_array($_REQUEST['account_type'],
        array('user_money', 'frozen_money', 'rank_points', 'pay_points', 'fh_num', 'credit','credit2')))
    {
        $account_type = '';
    }
    else
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $fh_num   = floatval($_POST['add_sub_fh_num']) * abs(floatval($_POST['fh_num']));
    $credit   = floatval($_POST['add_sub_credit']) * abs(floatval($_POST['credit']));
	$credit2   = floatval($_POST['add_sub_credit2']) * abs(floatval($_POST['credit2']));

    if ($user_money == 0 && $frozen_money == 0 && $rank_points == 0 && $pay_points == 0&& $fh_num == 0 &&$credit == 0 &&$credit2 == 0)
    {
        sys_msg($_LANG['no_account_change']);
    }

    /* 保存 */
   // log_account_change($user_id, $user_money, $frozen_money, $rank_points, $pay_points, $fh_num, $credit, $change_desc, ACT_ADJUSTING);
   log_account_change_new($user_id, $user_money, $frozen_money, $rank_points, $pay_points, $fh_num, $credit,$credit2,  $change_desc, ACT_ADJUSTING);
...
function get_accountlist($user_id, $account_type = '')
{
    ...
    if (in_array($account_type, array('user_money', 'frozen_money', 'rank_points', 'pay_points', 'fh_num', 'credit','credit2')))
    {
