...
//关闭股份
    ...
    $smarty->assign('action_link',   array('text' => $_LANG['surplus_add'], 'href'=>'user_accountgf.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('user_account_listgf.htm');
}
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    assign_query_info();
    $smarty->display('user_account_infogf.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$ecs->table('user_account').
               " VALUES ('', '$user_id', '$gufennum','$_SESSION[admin_name]', '$amount', '".gmtime()."', '$qixian','".gmtime()."', '$admin_note', '$user_note', '$process_type', '$payment', '$is_paid','$status','$realname','$mobile','$idcard',0,0,0)";
        $db->query($sql);
        $id = $db->insert_id();
    }
    else
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_account_listgf.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = " WHERE 1 and ua.gufennum>=1 and ua.process_type=0 and is_paid=1";
        if ($filter['user_id'] > 0)
        {
...
//                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
    ...
    file_put_contents(__DIR__."/1.txt", $sql);
    ...
    foreach ($list AS $key => $value)
    {
        ...
        if($value['tuigutime']){
            $list[$key]['tuigutime']             = local_date($GLOBALS['_CFG']['time_format'], $value['tuigutime']);
        }
        if($value['qixian']==6){
            $list[$key]['shijian']='半年';
        }elseif($value['qixian']==12){
            $list[$key]['shijian']='一年';
        }elseif($value['qixian']==60){
            $list[$key]['shijian']='五年';
        }
        if($value['status']==1){
            $list[$key]['status']='启用';
