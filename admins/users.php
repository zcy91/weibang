...
function action_edit ()
{
	...
	/* 代码增加2014-12-23 by www.68ecshop.com _star */
	$sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money,u.fh_num,u.credit, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
...
function action_update ()
{
	...
	$district = $_POST['district'];
	$address = empty($_POST['address']) ? '' : trim($_POST['address']);
	$mobile = empty($_POST['parent_id']) ? '' : trim($_POST['parent_id']);
	$status = $_POST['status'];
	/* 代码增加2014-12-23 by www.68ecshop.com _end */
	...
	$agentid=0;
	if(!empty($mobile)){
		$sql1 = "SELECT * FROM " . $GLOBALS['ecs']->table('users') .
			" WHERE mobile_phone = '$mobile'";
		$mobile_user = $GLOBALS['db']->getRow($sql1);
		if(!empty($mobile_user)){
			if($mobile_user['user_name']==$username){
				sys_msg("不能设置自己为上级", 1);
			}
			$agentid = $mobile_user['user_id'];
			//$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $me, "UPDATE", "user_name = '$username'");
		}else{
			sys_msg("会员不存在", 1);
		}
	}
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`parent_id`='$agentid',`status`='$status' where user_name = '" . $username . "'";
	$db->query($sql);
...
function user_list ()
{
	...
	if($result === false)
	{
        ...
        $filter['fh_num_gt'] = empty($_REQUEST['fh_num_gt']) ? 0 : intval($_REQUEST['fh_num_gt']);
        $filter['fh_num_lt'] = empty($_REQUEST['fh_num_lt']) ? 0 : intval($_REQUEST['fh_num_lt']);
		$filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'user_id' : trim($_REQUEST['sort_by']);
		$filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
        /*file_put_contents(__DIR__.'/a.php',json_encode($filter));*/
		$ex_where = ' WHERE 1 ';
		if($filter['keywords'])
		{
		...
		if($filter['rank'])
		{
			...
			{
				$ex_where .= " AND user_rank_points >= " . intval($row['min_points']) . " AND user_rank_points < " . intval($row['max_points']);
			}
		}
        ...
        if($filter['fh_num_gt'])
        {
            $ex_where .= " AND fh_num >= '$filter[fh_num_gt]' ";
        }
        if($filter['fh_num_lt'])
        {
            $ex_where .= " AND fh_num < '$filter[fh_num_lt]' ";
        }
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, validated,fh_num, user_money, frozen_money, rank_points,user_rank_points, pay_points, status, reg_time, froms ".
		        /* 代码增加2014-12-23 by www.68ecshop.com  _end  */
	...
	for($i = 0; $i < $count; $i ++)
	{
		...
		for ($j = 0; $j < count($rank_list); $j ++)
		{
			...
			$rank_points = $user_list[$i]['user_rank_points'];
